...
DESIGN=$(ls -t ./dstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ./dstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ./dstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ./dstack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ./dstack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ./dstack/projects/$SLUG/ceo-plans/{old-plan}.md ./dstack/projects/$SLUG/ceo-plans/archive/
```

Write to `./dstack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ./dstack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
rm -f ./dstack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`./dstack/` (user config directory, not project files). The skill preamble
already writes to `./dstack/sessions/` and `./dstack/analytics/` — this is
...
- **B)** Keep in `./dstack/projects/` only (local, personal reference)
- **C)** Skip
