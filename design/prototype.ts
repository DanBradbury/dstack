 ...
 *
 * Run: OPENAI_API_KEY=$(cat ./dstack/openai.json | python3 -c "import sys,json;print(json.load(sys.stdin)['api_key'])") bun run design/prototype.ts
 */
...
if (!API_KEY) {
  console.error("No API key found. Set OPENAI_API_KEY or save to ./dstack/openai.json");
  process.exit(1);
}
